...
export function PDFViewer({ fileUrl, title, className }: PDFViewerProps) {
  ...
  const openFullscreen = () => window.open(fileUrl, '_blank');
  ...
  return (
    <div className={cn('flex flex-col rounded-lg border border-border bg-card overflow-hidden', className)}>
      {/* Controls */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/50 px-2 py-2 md:px-4 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-1 md:gap-2">
          ...
          <Button variant="outline" size="sm" onClick={zoomIn} disabled={scale >= 2.5} className="h-8 w-8 p-0">
            <ZoomIn className="h-4 w-4" />
          </Button>
          <Button variant="outline" size="sm" onClick={openFullscreen} title="Open in new tab" className="h-8 w-8 p-0">
      ...
      <div className="relative overflow-auto bg-muted/30 p-2 md:p-4" style={{ maxHeight: '80vh' }}>
        {loading && (
